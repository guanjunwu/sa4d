...
from utils.loss_utils import l1_loss, ssim, l2_loss, lpips_loss, reg3d_loss
...
def training(dataset, hyper, opt, pipe, mode="feature", testing_iterations=None, saving_iterations=None, checkpoint_iterations=None, checkpoint=None, debug_from=None):    
    ...
    for iteration in range(first_iter, final_iter + 1):   
        ...
        dino_features = viewpoint_cam.dino_feature.cuda()
        rendered_features = render_contrastive_feature(viewpoint_cam, gaussians, pipe, background)["render"]
        H, W = rendered_features.shape[-2:]
        dino_features = torch.nn.functional.interpolate(dino_features[None, ...], size=(H, W), mode='bilinear').squeeze(0)
        rendered_features = gaussians._mlp(rendered_features.view(-1, H*W).permute([1, 0])).squeeze().permute([1,0]).reshape([-1, H, W])
        loss_l2 = l2_loss(rendered_features, dino_features)
        
        loss_3dreg = None
        if iteration % 5 == 0:
            # regularize at certain intervals
            features = gaussians._mlp(gaussians.get_sam_features)
            loss_3dreg = reg3d_loss(gaussians._xyz.squeeze().detach(), features)
            loss = loss_l2 + loss_3dreg
        else:
            loss = loss_l2
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                if loss_3dreg is not None:
                    progress_bar.set_postfix({"loss": f"{loss.item():.{4}f}",
                                            "loss_l2": f"{loss_l2.item():.{4}f}",
                                            "loss_3dreg": f"{loss_3dreg.item():.{4}f}",})
                else:
                    progress_bar.set_postfix({"loss_l2": f"{loss_l2.item():.{4}f}"})
