...
def rotate_by_euler_angles(means3d, rotations, rotation_angles):
        ...
        rotation_matrix = np.asarray(rx(x) @ ry(y) @ rz(z), dtype=np.float32)
...
def rotate_by_matrix(means3d, rotations, rotation_matrix, keep_sh_degree: bool = True):
    # rotate xyz
    means3d = np.asarray(np.matmul(means3d, rotation_matrix.T))
    ...
    rotations = rotations_from_quats / np.linalg.norm(rotations_from_quats, axis=-1, keepdims=True)
