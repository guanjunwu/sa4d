...
def render(viewpoint_camera, gaussians, bg_color : torch.Tensor, scaling_modifier = 1.0, motion_bias = [torch.tensor([0,0,0])], rotation_bias = [torch.tensor([0,0])],
    ...
    for index, pc in enumerate(gaussians):
        ...
        if index == 0:
            means3D_final, scales_final, rotations_final, opacity_final, shs_final = get_state_at_time(pc, viewpoint_camera)
        else:
            means3D_final1, scales_final1, rotations_final1, opacity_final1, shs_final1 = get_state_at_time(pc, viewpoint_camera, seg=True)
...
parser.add_argument("--configs1", type=str, default="arguments/hypernerf/default.py")
parser.add_argument("--configs2", type=str, default="arguments/hypernerf/default.py")
parser.add_argument("--modelpath2", type=str, default="output/hypernerf/split-cookie")
...
render_path = "output/editing_render_chickchicken"
...
gaussians2.load_mask_table(os.path.join(args2.model_path, "segment_results"))
...
for index, viewpoint in tqdm(enumerate(scene1.getVideoCameras())):
    result = render(viewpoint, gaussians, 
                    bg_color=background,
                    motion_bias=[torch.tensor([0, 2, -2])],
