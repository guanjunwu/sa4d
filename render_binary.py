...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, cam_type):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        # render image
        render_rgb = render(view, gaussians, pipeline, background, cam_type=cam_type)["render"]
        rgb = to8b(render_rgb).transpose(1,2,0)
        
        # render mask
        render_logits = render_contrastive_feature(view, gaussians, pipeline, background)["render"]
        prob = render_logits
        # prob = torch.sigmoid(render_logits)
        mask = (prob > 0.5).squeeze().cpu().numpy()
        pred_mask = (prob > 0.5).float().squeeze().cpu().numpy()
        
        
        # display mask on the image
        mask_rgb = rgb
        mask_rgb[mask] = (opacity * (pred_mask[:, :, None] * mask_color[None, None, ...]) + (1 - opacity) * rgb)[mask]
        # mask_rgb = opacity * (pred_mask[:, :, None] * mask_color[None, None, ...]) + (1 - opacity) * rgb
        
        pred_mask_list.append(mask_rgb.astype(np.uint8))

    imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'video_mask.mp4'), pred_mask_list, fps=30)
